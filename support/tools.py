...
def get_refund_history(user_id):
    refunds=RefundRequest.objects.filter(id=user_id).order_by('-created_at')
    ...
    for refund in refunds:
        history.append({
            ...
            "request_on":refund.created_at.strftime("%d %b %Y"),# 14 jan 2026
        })
    return {
...
def check_delivery_status(tracking_number,carrier):
    default_response={
        ...
        "last_update": "N/A",
        "estimated_delivery": "Contact to carrier directly",
        "delay_reason": "No update from the carrier",
    }
